...
void all_off_generate(const char* read_this_backup_file){

  ofstream hv_inputs(Form("HV_caget.sh"));
  
  for(int use_this_hv_crate = 31; use_this_hv_crate < 42; use_this_hv_crate++){
    if(use_this_hv_crate==37){
      continue;
    }
  ...
  */
	...
	hv_check.Form("caput HAHV%i:%02i:%03i:Pw 0", ind1_arr[cell], ind2_arr[cell], ind3_arr[cell]);
	
	hv_inputs << hv_check  << endl;
